      ...
      {items.map((item, i) => {
        ...
        return (
          <div key={i} style={{ marginBottom: 26 }} className="animate-pop">
            <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderLeft: '3px solid #0B2447', borderRadius: 12, padding: '14px 16px' }}>
              <div style={{ font: "700 11px 'Libre Franklin'", letterSpacing: '.12em', textTransform: 'uppercase', color: '#0B2447', marginBottom: 6 }}>{item.tag}</div>
              <div className="serif" style={{ fontSize: 16, lineHeight: 1.55, color: '#1E2D40' }}>{item.text}</div>
            </div>
            <div style={{ font: "600 16px/1.4 'Libre Franklin'", color: '#16263D', margin: '15px 0 12px' }}>{i + 1}. {item.prompt}</div>
